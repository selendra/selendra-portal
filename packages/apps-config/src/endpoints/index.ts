...
// import { prodRelayKusama, prodRelayPolkadot } from './production.js';
...
export function createWsEndpoints (t: TFunction = defaultT, firstOnly = false, _withSort = true): LinkOption[] {
  // Find Selendra endpoint from prodChains
  const selendraEndpoint = prodChains.find(({ info }) => info === 'selendra');
  ...
  return [
    ...createCustom(t),
    {
      ...
      isHeader: true,
      isSpaced: true,
      text: t('rpc.header.selendra', 'Selendra Network', { ns: 'apps-config' }),
      textBy: '',
      ui: {},
      value: ''
    },
    // Add Selendra as primary network
    ...(selendraEndpoint ? expandEndpoints(t, [selendraEndpoint], firstOnly, false) : []),
    // Comment out other networks for simplicity
    // {
    //   isDisabled: false,
    //   isHeader: true,
    //   isSpaced: true,
    ...
    //   value: ''
    // },
    // ...expandEndpoints(t, prodChains.filter(({ info }) => info !== 'selendra'), firstOnly, withSort),
    // {
    //   isDisabled: false,
    //   isHeader: true,
    //   isSpaced: true,
