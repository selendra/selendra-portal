...
function getApiUrl (): string {
  ...
  const selendraEndpoint = endpoints.find(({ info }) => info === 'selendra');
  const fallbackUrl = selendraEndpoint || endpoints.find(({ value }) => !!value);

  // via settings, or the default chain (Selendra by default)
  return [stored.apiUrl, process.env.WS_URL].includes(settings.apiUrl)
    ? settings.apiUrl // keep as-is
    : fallbackUrl
      ? fallbackUrl.value // grab the fallback (Selendra preferred)
      : 'wss://rpc.selendra.org'; // default to Selendra
}
